	PROGRAM RK4_SISTEMA
	DIMENSION YA(0:10), YN(0:10), EK(0:4), Y(0:10),Ka(5),Le(5),YK(5),YL(5)
        integer s
        real w
	PRINT *
...
!se definen las condiciones iniciales
	...
	Y(1) = 0.3
	Y(2) = 0.1
        open(unit=20,file='rk4d.txt',status='unknown')
...
F=0
 C=0.5
  w=(3.2/5)**2
if (XP .LT. 1 .or. XP .eq. 1) then
	EK(1) =  YA(2)
	EK(2) = -2*C*w*YA(2)-(w**2)*YA(1)+1/5.
		...
		YK(2) = H*(EK(1)+H/2)
                EK(2)=-2*C*w*(YA(2)+H/2)-(w**2)*(YA(1)+YK(1)/2)+1/5.
                YL(2) = H*(EK(2))


		J = 3
		YK(3) = H*(EK(1)+H/2)
		EK(2)=-2*C*w*(YA(2)+H/2)-(w**2)*(YA(1)+YK(2)/2)+1/5.	
		YL(3)=H*(EK(2))
		XA = XM
	
		J = 4
		YK(4) = H*(EK(1)+YL(3))
		EK(2)=-2*C*w*(YA(2)+YL(3))-(w**2)*(YA(1)+YK(3)/2)+1/5.
		YL(4)=H*EK(2)
...
98	FORMAT(1X,I2,F10.6,2X,1P4E16.6/(15X,1P4E16.6))
	...
	EK(2) = -2*C*w*YA(2)-(w**2)*YA(1)
		...
		YK(2) = H*(EK(1)+H/2)
                EK(2)=-2*C*w*(YA(2)+H/2)-(w**2)*(YA(1)+YK(1)/2)
                YL(2) = H*(EK(2))


		J = 3
		YK(3) = H*(EK(1)+H/2)
		EK(2)=-2*C*w*(YA(2)+H/2)-(w**2)*(YA(1)+YK(2)/2)	
		YL(3)=H*(EK(2))
		XA = XM
