...
Puede usar varios métodos para calcularlas:
    0=bisect (por default)
    1=newton_raphson (Si se usa éste hay que dar el valor de la
        función derivada df)
    2=secante
"""
def toda_raiz(f,a,b,dx,epsilon=1e-9,prn=0,plot=0,metodo="bisect",df=0):
    raiz=[]
    x1=a
    k=1
    k_no_sing=1
    while x1<b:
        ...
        if   metodo=="bisect":          xraiz=bisect(f,x1,x2,0,epsilon)
        elif metodo=="newton_raphson":  xraiz=newton_raphson(f,df,x1,x2,epsilon)
        elif metodo=="secante":         xraiz=secante(f,x1,x2,epsilon)
        if xraiz!=None: raiz.append(xraiz)
        decformat="%."+str(int(abs(round(log10(epsilon)))))+"f"
        decformat_intervalo="\nLa raíz en el intervalo ("+decformat+","+decformat+") es:"
        if prn==2: print decformat_intervalo %(x1,x2)
        if prn>=1: print "x",k_no_sing,"=",decformat %raiz[k-1]
...
def bisect(f,x1,x2,switch=0,epsilon=1e-9):
    f1=f(x1)
    if f1==0.: return x1
    f2=f(x2)
    if f2==0.: return x2
