...
def grafica(f,a,b,dx):          #Rutina para graficar f
    ...
    for i in xdominio:
        yrango.append(f(i))
    plt.plot(xdominio,yrango,"b-")
    plt.title(u'Función para el problema 9-c')
    plt.axhline(y=0,c='k')
    plt.axvline(x=0,c='k')
    plt.text(-4,5,u'$x_{1}=-3.104$',fontsize=20)
    #plt.text(1.5,-0.4,u'$x_{2}=3.3155$',fontsize=20)
    #plt.text(20,11000,u'$x_{3}=0.8530$',fontsize=20)
    #plt.text(20,9000,u'$x_{4}=50.183$',fontsize=20)
    plt.grid()
    plt.show()
    return
